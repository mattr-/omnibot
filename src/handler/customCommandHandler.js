...
export class CustomCommandHandler extends BaseHandler {
  async check(cmd) {
    try {
      ...
      const isAllowed = this.options && message.member && !isEmpty(intersection(this.options.roles, message.member.roles.map((role) => role.name))) || isEmpty(this.options.roles) || this.options.roles === undefined;
